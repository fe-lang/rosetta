...
contract ProofParseDebugTest {
    ...
    function test_proofLAtZeta() public view {
        // Strip selector, append raw proof to calldata
        bytes memory rawProof = new bytes(PROOF.length - 4);
        for (uint i = 0; i < rawProof.length; i++) rawProof[i] = PROOF[i + 4];

        bytes memory callData = abi.encodePacked(
            bytes4(keccak256("debugProofL(uint256)")),
            uint256(36),  // proof starts right after the 1 param (4 + 32 = 36)
            rawProof
        );

        (bool success, bytes memory result) = feAddr.staticcall(callData);
        require(success, "debugProofL reverted");
        uint256 lAtZeta = abi.decode(result, (uint256));

        // Expected: 0x2a2af70b568d007ce53077a078437c2acf6cad206354b0ffa823b4de87918a05
        require(
            lAtZeta == 0x2a2af70b568d007ce53077a078437c2acf6cad206354b0ffa823b4de87918a05,
            "l_at_zeta mismatch"
        );
    }
}
